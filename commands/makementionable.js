...
module.exports.run = async (client, msg, args) => {
  ...
  const role = msg.guild.roles.find(r => r.name === args[0]) || msg.guild.roles.get(args[0])
  if (!role) return msg.channel.send("Role not found.")
  role.setMentionable(true).then(r => {msg.channel.send("Role is now mentionable for 10 seconds").then(m => m.delete(5000)), setTimeout(() => {r.setMentionable(false)}, 10000)})
}
